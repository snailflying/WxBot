...
//        XposedBridge.log("aaron1 ClassTrie mutable:$mutable")
...
//            XposedBridge.log("aaron1 ClassTrie search packageName:$packageName, depth:$depth, pos:$pos, delimiterAt$delimiterAt")

            if (delimiterAt == -1) {
                val pkg = packageName.substring(pos)
//                XposedBridge.log("aaron1 ClassTrie search packageName:$packageName")
//                XposedBridge.log("aaron1 ClassTrie search pkg:$pkg")
//                XposedBridge.log("aaron1 ClassTrie search children:${children[pkg]?.get(depth)},size:${children[pkg]?.get(depth)?.size}")
...
//            XposedBridge.log("aaron1 ClassTrie search list:${next.search(packageName, depth, delimiterAt + 1)[0]}")
