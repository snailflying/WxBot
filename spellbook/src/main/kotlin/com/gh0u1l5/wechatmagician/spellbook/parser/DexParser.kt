...
import com.gh0u1l5.wechatmagician.spellbook.util.Buffers
...
class DexParser(buffer: ByteBuffer) {
    ...
    private fun readDexHeader() = DexHeader().apply {
        ...
        fileSize = Buffers.readUInt(buffer)
        headerSize = Buffers.readUInt(buffer)

        endianTag = Buffers.readUInt(buffer)

        linkSize = Buffers.readUInt(buffer)
        linkOff = Buffers.readUInt(buffer)

        mapOff = Buffers.readUInt(buffer)

        stringIdsSize = buffer.int
        stringIdsOff = Buffers.readUInt(buffer)

        typeIdsSize = buffer.int
        typeIdsOff = Buffers.readUInt(buffer)

        protoIdsSize = buffer.int
        protoIdsOff = Buffers.readUInt(buffer)

        fieldIdsSize = buffer.int
        fieldIdsOff = Buffers.readUInt(buffer)

        methodIdsSize = buffer.int
        methodIdsOff = Buffers.readUInt(buffer)

        classDefsSize = buffer.int
        classDefsOff = Buffers.readUInt(buffer)

        dataSize = buffer.int
        dataOff = Buffers.readUInt(buffer)
    }

    private fun readStringOffsets(stringIdsOff: Long, stringIdsSize: Int): IntArray {
    ...
    private fun readTypeIds(typeIdsOff: Long, typeIdsSize: Int): IntArray {
    ...
    private fun readClassIds(classDefsOff: Long, classDefsSize: Int): Array<Int> {
