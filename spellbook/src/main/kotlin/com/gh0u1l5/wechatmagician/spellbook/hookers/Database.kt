...
object Database : EventCenter() {
    ...
    private val onExecuteHooker = Hooker {
        findAndHookMethod(
                ...
                C.String, C.ObjectArray, object : XC_MethodHook() {
            @Suppress("UNCHECKED_CAST")
            override fun beforeHookedMethod(param: MethodHookParam) {
                val thisObject   = param.thisObject
...
//                val cancellation = param.args[2]
                notifyForBypassFlags("onDatabaseExecuting", param) { plugin ->
                    (plugin as IDatabaseHook).onDatabaseExecuting(thisObject, sql, bindArgs)
                }
            }
            @Suppress("UNCHECKED_CAST")
            override fun afterHookedMethod(param: MethodHookParam) {
...
//                val cancellation = param.args[2]
                notify("onDatabaseExecuted") { plugin ->
                    (plugin as IDatabaseHook).onDatabaseExecuted(thisObject, sql, bindArgs)
                }
            }
        })
    }
