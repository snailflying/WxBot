...
//    }
    ...
    fun formatPhoneNum(phoneNum: String?): String {
        val phoneNoPattern = "^((\\+?86)?\\s?-?)1[0-9]{10}"
        if (phoneNum?.matches(phoneNoPattern.toRegex()) == true) {
...
//    }
    ...
    /**
     * 根据zoneCode检测字符串是否是手机号，大陆手机号前面可以带或者不带“+86”、“86”
     * @param phoneNum 被检测值
     * @param zoneCode 国家区号
     * @return Boolean
     */
    fun isPhoneNo(phoneNum: String?, zoneCode: String? = "86"): Boolean {
        val cnPattern = "^(\\+?86)"
        val phoneNoPattern = "^((\\+?86)?\\s?-?)1[0-9]{10}"

        return when {
            zoneCode?.matches(cnPattern.toRegex()) == true -> {
                return phoneNum?.matches(phoneNoPattern.toRegex()) == true
            }
            else -> true
        }
...
//                val matcher = ip.matcher(hostAddress)
    ...
    fun isDebug(context: Context = App.instance): Boolean {
        return 0 != (context.applicationInfo.flags and ApplicationInfo.FLAG_DEBUGGABLE)
    }

    fun getTopDomain(host: String?): String? {
        if (host == null) return null

        val pattern = Pattern.compile("[^.]+(\\.com\\.cn|\\.net\\.cn|\\.org\\.cn|\\.gov\\.cn|\\.com|\\.net|\\.cn|\\.org|\\.cc|\\.me|\\.tel|\\.mobi|\\.asia|\\.biz|\\.info|\\.name|\\.tv|\\.hk|\\.公司|\\.中国|\\.网络)$");
